...
namespace md2visio.GUI.Forms
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void OnSelectedFileChanged(object? sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(_selectedFilePath))
            {
                string safePath = _selectedFilePath.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
                var fileName = Path.GetFileNameWithoutExtension(safePath);

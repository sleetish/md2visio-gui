...
namespace md2visio.GUI.Services
{
    ...
    public class ConversionService : IDisposable
    {
        ...
        private string BuildOutputPath(string outputDir, string? fileName)
        {
            ...
            {
                // 🛡️ Sentinel: Prevent path traversal by sanitizing fileName
                string sanitizedName = fileName.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);

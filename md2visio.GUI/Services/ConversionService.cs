    ...
    public class ConversionService : IDisposable
    {
        ...
        private string BuildOutputPath(string outputDir, string? fileName)
        {
            if (!string.IsNullOrEmpty(fileName))
            {
                ...
                string sanitized = fileName.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
                sanitized = Path.GetFileName(sanitized);

                if (string.IsNullOrWhiteSpace(sanitized) || sanitized == ".vsdx")
                {
                    sanitized = "output.vsdx";
                }
                else if (!sanitized.EndsWith(".vsdx", StringComparison.OrdinalIgnoreCase))
                {
                    sanitized += ".vsdx";
                }
                return Path.Combine(outputDir, sanitized);
            }
            return outputDir;
        }

...
namespace md2visio.GUI.Services
{
    ...
    public class ConversionService : IDisposable
    {
        ...
        private string BuildOutputPath(string outputDir, string? fileName)
        {
            ...
            {
                // Normalize slashes for cross-platform compatibility and mitigate path traversal
                string sanitizedName = fileName.Replace('/', Path.DirectorySeparatorChar)
                                             .Replace('\\', Path.DirectorySeparatorChar);
                sanitizedName = Path.GetFileName(sanitizedName);

                // Fallback to a safe default if the sanitized name is empty

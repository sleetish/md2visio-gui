...
namespace md2visio.GUI.Services
{
    ...
    public class ConversionService : IDisposable
    {
        ...
        private string BuildOutputPath(string outputDir, string? fileName)
        {
            ...
            {
                // 🛡️ Sentinel: Prevent path traversal by sanitizing the user-provided filename
                // Normalize slashes and extract just the filename
                string sanitizedFileName = fileName.Replace('\\', Path.DirectorySeparatorChar)

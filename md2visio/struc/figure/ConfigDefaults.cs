...
namespace md2visio.struc.figure
{
    internal class ConfigDefaults: IConfig
    {
        ...
        {
            DarkMode = darkMode;
            string safeTheme = theme2load.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
            string sanitizedTheme = Path.GetFileName(safeTheme).ToLower();

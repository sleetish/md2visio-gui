...
namespace md2visio.struc.figure
{
    internal class ConfigDefaults: IConfig
    {
        ...
        {
            DarkMode = darkMode;
            string safePath = theme2load.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
            string sanitizedTheme = Path.GetFileName(safePath).ToLower();

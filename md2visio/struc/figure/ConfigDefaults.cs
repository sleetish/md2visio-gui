...
namespace md2visio.struc.figure
{
    internal class ConfigDefaults: IConfig
    {
        ...
        {
            DarkMode = darkMode;
            // Sanitize cross-platform slashes to prevent path traversal
            string normalizedTheme = theme2load.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);

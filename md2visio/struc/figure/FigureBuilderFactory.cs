...
namespace md2visio.struc.figure
{
    internal class FigureBuilderFactory
    {
        ...
        void InitOutputPath()
        {
            ...
            {
                isFileMode = true;
                string sanitizedOutput = outputFile.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
                name = Path.GetFileNameWithoutExtension(sanitizedOutput);
            ...
            {
                isFileMode = false;
                string sanitizedInput = iter.Context.InputFile?.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar) ?? string.Empty;
                name = Path.GetFileNameWithoutExtension(sanitizedInput);
                if (string.IsNullOrEmpty(name)) name = "output";

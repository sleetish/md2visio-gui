...
namespace md2visio.struc.figure
{
    internal class MmdJsonObj : ValueAccessor
    {
        ...
        {
            if (depth > MAX_DEPTH) throw new InvalidOperationException("Maximum JSON recursion depth exceeded.");
            this.index = index;
        ...
        MmdJsonObj UpdateWith(MmdJsonObj json, StringBuilder path, int depth = 0)
        {
            if (depth > MAX_DEPTH) throw new InvalidOperationException("Maximum JSON recursion depth exceeded.");
